...
tag1='Legacy2018_12Oct2021'
#tag2='Legacy2018_06Apr2021'
...
massval = np.array([270,300,350,400,450,500,550,600,650,700,750,800,850,900,1000])

#Compare with the analysis using data in 2016
#oldlimit = [160.6,356.4,309.6,335.0,229.5,106.0,60.1,41.5,28.8,20.0,14.2,10.3,6.3]

#Compare with the results from YX analysis
oldlimit = [220.0793689,156.9339427,79.98270708,43.49846725,25.57012208,19.19551457,15.57227551,12.15975145,10.25782237,8.805061299,7.806656742,6.922731297,6.629265775,6.497792458,5.919786961]

#Compare with ATLAS results
massval2 = np.array([254.6090589,263.1881057,282.1297037,303.3202097,328.5756738,353.837647,377.3844618,400.9573131,449.7693556,500.361648,551.8945112,600.1565314,700.0425427,797.5299358,899.9772938,998.3466755,1099.274149,1199.342416,1399.651443,1598.339694])
limits2 = np.array([26.02320138,52.92035574,61.27559974,47.66204621,34.65722505,25.51118825,16.31348552,10.95534421,5.188441226,3.196881812,2.470319327,1.944352511,1.403604008,1.138231359,0.99329611,0.904814052,0.990285988,1.007099935,1.44067188,2.1380])


grexp2 = ROOT.TGraph()#(len(massval2),massval2,limits2)

ptsList = [] # (x, obs, exp, p2s, p1s, m1s, m2s)

for im,mass in enumerate(massval):
   ...
   exp1  = 10.*parseFile(fName)*1.2896
...
#for im,mass in enumerate(massval2):
#	index=np.digitize(np.array([mass]),massval)[0]
#	exp2=(ptsList[index][1]-ptsList[index-1][1])*(mass-massval[index-1])/(massval[index]-massval[index-1])+ptsList[index-1][1]
#	ratio.SetPoint(im,mass,exp2/limits2[im])
#	print exp2
...
grexp2.SetMarkerColor(ROOT.kAzure+10)
...
legend.AddEntry(grexp1, "Results in this analysis", "l")
legend.AddEntry(grexp2, "H#rightarrowhh_{s} results", "l")
...
pt = ROOT.TPaveText(0.1663218-0.06,0.886316,0.3045977-0.02,0.978947,"brNDC")
pt.SetBorderSize(0)
pt.SetTextAlign(12)
pt.SetTextFont(62)
...
pt2 = ROOT.TPaveText(0.736111-0.2,0.9066667,0.847222,0.954641,"brNDC")
pt2.SetBorderSize(0)
pt2.SetFillColor(0)
pt2.SetTextSize(0.045)
pt2.SetTextFont(42)
pt2.SetFillStyle(0)
pt2.AddText("Scaled to a luminosity of 35.9 fb^{-1} (13 TeV)")
...
pt5.AddText('#scale[0.8]{Combined categories}')
#pt5.AddText(sel)
...
hframe = ROOT.TH1F('hframe', '', 100, 250, 1000)
...
if '2016' in tag1:
   ...
   hframe.SetMinimum(0.08)
   hframe.SetMaximum(800)#4000)
...
hframe2 = ROOT.TH1F('hframe', '', 100, 250, 1000)
hframe2.SetMinimum(0)
hframe2.SetMaximum(1.3)
...
hframe2.GetYaxis().SetTitle("#frac{New results}{H#rightarrowhh_{S} results}")
hframe2.SetStats(0)
...
line=ROOT.TLine(250,0.7755,900,0.7755)
line.SetLineWidth(3)
...
c1.SaveAs("plots/massScan_withhhs_combined"+".pdf",'pdf')
c1.SaveAs("plots/massScan_withhhs_combined"+".png")
# for m in masses :
