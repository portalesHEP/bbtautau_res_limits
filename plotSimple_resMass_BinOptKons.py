...
for ch in ['ETau','MuTau','TauTau']:
	for ca in selections:
		...
		for mass in massval:
		   ...
		   fName = 'cards_{0}{1}/{2}/combined_out/comb.Radion{3}.log'.format(ch,tag1,ca,mass)
		   exp1  = 10.*parseFile(fName)
		   fName = 'cards_{0}{1}/{2}/combined_out/comb.Radion{3}.log'.format(ch,tag2,ca,mass)
		...
		legend.AddEntry(grexp1, "Bayesian optimization", "l")
		legend.AddEntry(grexp2, "Results with old binnings", "l")
		...
		hframe2.GetYaxis().SetTitle("#frac{Bayesian Opt}{Old binnings}")
		hframe2.SetStats(0)
		...
		c1.SaveAs("plots/massScan_BinOptKons_"+ch+"_"+ca+".pdf","pdf")
# for m in masses :
